...
public class DiscountDetailActivity extends BaseActivity {
	...
	private TextView textPrice, order_count;
	...
	private void showShare() {
		...
		if (MyApplication.getInstance().getStringGlobalData("session", null) != null) {
			...
			oks.setText(discount.getShop_name()+","+discount.getTitle() + " 更多优惠服务尽车粒子洗车平台。车粒子下载地址："
	...
	private void reloadData() {
		String url = Const.SERVER_BASE_PATH + Const.DISCOUNT;
		GOVHttp request = GOVHttp.requestWithURL(url, "POST");
		request.setValueForKey("discount_id", "" + discount.getDiscount_id());
		...
		jsonObj = (JSONObject) request
				.execForJSONObject(DiscountDetailActivity.this);
	...
	private static class DiscountDetailHandler extends Handler {
		WeakReference<DiscountDetailActivity> mActivity;

		DiscountDetailHandler(DiscountDetailActivity activity) {
			mActivity = new WeakReference<DiscountDetailActivity>(activity);
		}

		@Override
		public void handleMessage(Message msg) {
			DiscountDetailActivity theActivity = mActivity.get();
			...
			case SUPPORT_REPLY:
				...
				try {
					...
					if (jsonObject.optInt("status") == 1) {
						...
						theActivity.discount.setFollowed(fBoolean);
						theActivity.discount.setFollow_count(jsonObject
								.optInt("follow_count"));
	...
	public void createPopupWindow() {
		...
		if (popupWindow == null) {
			...
			View view = LayoutInflater.from(mContext).inflate(
					R.layout.order_popup_view_layout, null);
	...
	private void updatePopuoView() {
		...
		lefTextView.setText(discount.getOrder_count() + "");
	...
	protected String httpOrder() throws Exception {
		// TODO Auto-generated method stub
		String url = Const.SERVER_BASE_PATH + Const.ORDER;
		GOVHttp request = GOVHttp.requestWithURL(url, "POST");
		request.setValueForKey("discount_id", "" + discount.getDiscount_id());
		...
		if (MyApplication.getInstance().getStringGlobalData("session", null) != null) {
			...
			ThreadPoolUtils.execute(new Runnable() {
				...
				public void run() {
					...
					msg.what = ORDER_END;
					try {
						String result = DiscountDetailActivity.this.httpOrder();
						msg.obj = result;
